function searchContacts() {
    ...
    // Fetch data from the JSON endpoint (updated URL)
    fetch('https://jade091104.github.io/Activity-3-Contact-Search/name.json') // Corrected URL to name.json
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            // Ensure you're accessing the correct "contacts" data
            const contacts = data.contacts || []; // Access contacts properly from JSON response
    ...
    contacts.forEach(contact => {
        html += ` 
            <tr>
                ...
                <td>
                    ${contact.contactInfo?.email ? `<div><i class="fas fa-envelope me-2 text-primary"></i> ${contact.contactInfo.email}</div>` : ''}
                    ${contact.contactInfo?.phone ? `<div><i class="fas fa-phone me-2 text-primary"></i> ${contact.contactInfo.phone}</div>` : ''}
                </td>
                <td>
                    ${contact.address ? `
